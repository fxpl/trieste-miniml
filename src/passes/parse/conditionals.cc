  ...
  PassDef conditionals() {
    return {
      ...
      {(T(If) << (T(Group))[Cond]) * (T(Then) << (T(Group))[Then]) *
           (T(Else) << (T(Group))[Else]) >>
         [](Match& _) -> Node {
         return If << (Expr << *_[Cond]) << (Expr << *_[Then])
                   << (Expr << *_[Else]);
       },

       // Error rules
       T(If)[If] << --(T(Expr) * T(Expr) * T(Expr)) >> [](Match& _) -> Node {
         return err(
           _(If),
           "ill-formed if expression: expected 'if <cond> then <expr> else "
           "<expr>'");
       },
       ...
       T(Then)[Then] << End >> [](Match& _) -> Node {
         return err(_(Then), "expected expression after 'then'");
       },

       T(Else)[Else] << End >> [](Match& _) -> Node {
