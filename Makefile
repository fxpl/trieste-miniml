...
coverage ?= ON	#Enable code coverage (ON/OFF) statistics
mode = test     #trieste mode, either 'build' or 'test'
testcount = 100 #number of tests to run in test mode
out_dir = coverage_reports#directory to store coverage reports
...
llvm_path := /opt/homebrew/Cellar/llvm@20/20.1.8#path to llvm cmake files
clang++_path = $(llvm_path)/bin/clang++#path to clang++ compiler
src="src"
...
rebuild:
	...
	-DCMAKE_CXX_COMPILER=/opt/homebrew/Cellar/llvm@20/20.1.8/bin/clang++ \
	-DCMAKE_PREFIX_PATH=/opt/homebrew/Cellar/llvm@20/20.1.8 \
...
build: clean_build rebuild 
	@echo "Build completed"

ninja: 
	@cd build && ninja && echo "Ninja build completed"
...
llvm-cov export $(bin) \
  ...
  -format=text \
  -summary-only $(src) > $(llvm_cov_json)
endef
...
if [ "$(mode)" = "build" ]; then \
	...
	LLVM_PROFILE_FILE=$(profraw) $(bin) $(mode) -c $(testcount) parse_cleanup addsub ; \
fi && \
$(generate_json) && \
$(generate_html)
endef
...
clean: clean_build
	rm -rf out/* *.trieste; rm -rf out; rm out.ll; rm compiler_output.txt
